...
Page.getLayout = function getLayout(page: any) {
  return <AdminLayout title="Category Modul">{page}</AdminLayout>;
};
...
export default function Page() {
  ...
  const {
    data: dataCategoryModul,
    isLoading: isLoadingCategoryModul,
    total: totalCategoryModul,
    mutate: reloadCategoryModul,
  ...
  const onDeleteHandler = async (id: string) => {
    try {
      const result = await CategoryModulRepository.api.delete(id);
      notifications.show({
        title: 'Success',
        color: 'green',
          ...
          <Stack gap={'md'} id="table">
            ...
            <Table.ScrollContainer minWidth={500}>
              <Table verticalSpacing={'md'}>
                ...
                <tbody>
                  {dataCategoryModul?.map((item, index) => {
                    return (
