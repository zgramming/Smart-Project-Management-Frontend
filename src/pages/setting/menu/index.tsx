...
export default function Page() {
  ...
  const onDeleteHandler = async (id: string) => {
    try {
      const result = await MenuRepository.api.delete(id);
      notifications.show({
        title: 'Success',
        color: 'green',
                  ...
                  {dataMenu?.map((item, index) => {
                    return (
                      <Table.Tr key={item.id}>
                        ...
                        <Table.Td>{item.Modul.CategoryModul.name}</Table.Td>
                        <Table.Td>{item.Modul.name}</Table.Td>
                        <Table.Td>{item.ParentMenu?.name}</Table.Td>
